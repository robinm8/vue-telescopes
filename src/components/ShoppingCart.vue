    ...
    <div class="container">
        <div class="thumbnail">
            <table id="cart" class="table table-hover table-condensed">
                ...
                <tfoot>
                    <tr class="visible-xs">
                        <td class="text-center"><strong>Total {{ cartValue | toCurrency }}</strong></td>
                    </tr>
                    <tr>
                        <td colspan="2" class="hidden-xs"></td>
                        <td class="hidden-xs text-center"><strong>Total {{ cartValue | toCurrency }}</strong></td>
                        <td>
                    ...
                    for (let prodIdx = 0; prodIdx < prodList.length; prodIdx++) {
                        if (prodList[prodIdx].id == item.id) {
                            if (prodList[prodIdx].quantity < item.quantity) {
                                message = `We have only ${prodList[prodIdx].quantity} ${item.title} in stock.`;
                                isValid = false;
                                return;
                            }
                            break;
                ...
                if (this.isLoggedIn) {
                    ...
                    if (isValid) {
                        this.saveToTransaction({
                            ...
                            this.addMessage({
                                messageClass: 'success',
                                message: 'Your order has been placed! Status: CANCELED; Test Mode'
                            });
                            this.saveShoppingCart({
                                cartItemList: [],
                                uid: this.currentUser.uid
